...
\begin{itemize}
	\item \code{sorter.cpp}: it contains the main method and the \emph{FastFlow} business logic code. 
	\item \code{sorterLogic.hpp}: it contains the business logic code for the custom \emph{BSP} execution.
	\item \code{posixBSP.hpp}: it contains the implementation (with \emph{POSIX threads}) of the BSP pattern.
	\item \code{logicBSP.hpp}: it contains the interface that hosts the business logic code abstraction. Every application should subclass the one provided in this file to provide the business logic code.
	...
	\item \code{makefile}: the only thing worth noting is that the code is optimized with \code{-O3}, and the \emph{FastFlow} include path might have to be changed. The one provided works with the installation on the remote \emph{XEON PHI} machine. Just issuing \code{make all} should work.
\end{itemize}
